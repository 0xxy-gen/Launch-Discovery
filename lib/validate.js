...
export function validateLaunch(body = {}) {
  ...
  const priceLow       = blank(body.priceLow) ? null : num(body.priceLow);
  const priceHigh      = blank(body.priceHigh) ? null : num(body.priceHigh);
  const notes          = str(body.notes);
  ...
  // Price is optional — quoting on request is normal — but a half-filled range
  // is not, and a range that runs backwards is a typo worth catching.
  for (const [name, v] of [['priceLow', priceLow], ['priceHigh', priceHigh]]) {
    if (v !== null && (!Number.isFinite(v) || v < 0 || v > 1_000_000)) {
      fields[name] = 'Enter a price per kg, or leave both blank.';
    }
  }
  if (!fields.priceLow && !fields.priceHigh) {
    if ((priceLow === null) !== (priceHigh === null)) {
      fields[priceLow === null ? 'priceLow' : 'priceHigh'] = 'Give both ends of the range, or neither.';
    } else if (priceLow !== null && priceLow > priceHigh) {
      fields.priceHigh = 'The top of the range must not be below the bottom.';
    }
  }
  ...
  return {
    fields,
    values: { name, vehicle, operator, site, priceLow, priceHigh, orbitType, altitudeKm, inclinationDeg,
