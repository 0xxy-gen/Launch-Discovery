...
const stmt = {
  insert: db.prepare(`
    INSERT INTO launches (user_id, company_id, name, vehicle, operator, site, orbit_type, altitude_km,
                          inclination_deg, window_month, capacity_kg, committed_kg,
                          price_low, price_high, notes,
...
const cols = l => [
  ...
  l.windowMonth, l.capacityKg, l.committedKg,
  Number.isFinite(l.priceLow) ? l.priceLow : null,
  Number.isFinite(l.priceHigh) ? l.priceHigh : null,
  l.notes,
];
...
export function browseLaunches(filter = {}) {
  ...
  // Both of these are sets: an operator's real constraint is "US or EU", or
  // "SSO or polar", almost never exactly one value.
  if (filter.orbitTypes?.length) {
    where.push(`l.orbit_type IN (${filter.orbitTypes.map(() => '?').join(', ')})`);
    params.push(...filter.orbitTypes);
  }
  ...
  // Price: a flight quoting on request cannot be shown to be within budget, so
  // narrowing the ceiling drops it. Widen it back and it returns.
  if (filter.priceMin != null) { where.push('l.price_low >= ?'); params.push(filter.priceMin); }
  if (filter.priceMax != null) { where.push('l.price_low <= ?'); params.push(filter.priceMax); }

  if (filter.altMin != null) { where.push('l.altitude_km >= ?'); params.push(filter.altMin); }
  if (filter.altMax != null) { where.push('l.altitude_km <= ?'); params.push(filter.altMax); }
  ...
  if (filter.countries?.length) {
    where.push(`u.country IN (${filter.countries.map(() => '?').join(', ')})`);
    params.push(...filter.countries);
  }

  return db.prepare(`
    SELECT l.*, u.name AS provider, u.country AS provider_country, u.logo AS provider_logo
    FROM launches l
...
export function launchView(l) {
  return {
    ...
    name: l.name,
    vehicle: l.vehicle,
    // the source sheet has no site column; an unknown site is absent, not a value
    site: l.site && l.site !== 'Not stated' ? l.site : null,
    ...
    providerLogo: l.provider_logo || null,
    ...
    // null means the provider quotes on request, which most of them do
    priceLow: l.price_low ?? null,
    priceHigh: l.price_high ?? null,
    notes: l.notes,
  };
}
...
/** Orbit shells that actually have published flights, with counts. */
/** The bounds the sliders should span, from what is actually listed. */
export const launchBounds = () => db.prepare(`
  SELECT MIN(price_low) AS priceMin, MAX(price_low) AS priceMax,
         MIN(altitude_km) AS altMin, MAX(altitude_km) AS altMax
  FROM launches WHERE status = 'published'
`).get();

export const launchOrbits = () => db.prepare(`
  SELECT orbit_type AS orbitType, COUNT(*) AS n
  FROM launches
