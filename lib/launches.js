...
const stmt = {
  insert: db.prepare(`
    INSERT INTO launches (user_id, name, vehicle, site, orbit_type, altitude_km, inclination_deg,
  ...
  setStatus: db.prepare(
    'UPDATE launches SET status = ?, published_at = ? WHERE id = ? AND user_id = ?'),
  byId:    db.prepare('SELECT * FROM launches WHERE id = ?'),
  byOwner: db.prepare('SELECT * FROM launches WHERE user_id = ? ORDER BY window_month'),
  remove:  db.prepare('DELETE FROM launches WHERE id = ? AND user_id = ?'),
};
...
export function createLaunch(userId, l, publish) {
  const now = Date.now();
  const info = stmt.insert.run(
    userId, ...cols(l), publish ? 'published' : 'draft', now, publish ? now : null);
  return stmt.byId.get(info.lastInsertRowid);
}

export function updateLaunch(userId, id, l) {
  stmt.update.run(...cols(l), id, userId);
  return stmt.byId.get(id);
}

export function setLaunchStatus(userId, id, status) {
  stmt.setStatus.run(status, status === 'published' ? Date.now() : null, id, userId);
  return stmt.byId.get(id);
}

export const launchById      = id => stmt.byId.get(id);
export const launchesForOwner = userId => stmt.byOwner.all(userId);
export const deleteLaunch    = (userId, id) => stmt.remove.run(id, userId).changes > 0;
...
export function browseLaunches(filter = {}) {
  ...
  return db.prepare(`
    SELECT l.*, u.organisation AS provider, u.country AS provider_country
    FROM launches l
